...
class ForgeLite
{
    ...
    borderType: "solid" | "death" | Function = "solid";


    constructor(EM: EventManager = new EventManager(), CR: CanvasRenderer = new CanvasRenderer({}))
    {
        ...
        this.size = <SizeVector>CR.Size();
    };

    NewEntity(entity: PhysicsObject)
    {
    ...
    HandleGravity()
    {
        this.entities.forEach(i =>
        {
            if (i.noGravity) return;

            this.gravities.forEach(g =>
            {
                if (i.speed.Magnitude() < g.MaxSpeed) i.ApplyMotion(g.Direction.Normalized(), g.Power, false)
            })
        ...
        this.entities.forEach(i =>
        {
            ...
            if (!i.noCollide) {
                this.entities.forEach(e => {
                    if (e.noCollide || e === i) return;
                    if (!Collides(i, e)) return;
...
const DoLinesCollide = function (line1: { x: number, y: number }[], line2: { x: number, y: number }[])
{
    if (Math.max(line1[0].x, line1[1].x) < Math.min(line2[0].x, line2[1].x)) return false;
    if (Math.max(line2[0].x, line2[1].x) < Math.min(line1[0].x, line1[1].x)) return false;
    if (Math.max(line1[0].y, line1[1].y) < Math.min(line2[0].y, line2[1].y)) return false;
    if (Math.max(line2[0].y, line2[1].y) < Math.min(line1[0].y, line1[1].y)) return false;
    ...
    if (interceptX < interval[0] || interceptX > interval[1]) return false;

    return true;
};
const Collides = function (obj1: PhysicsObject, obj2: PhysicsObject)
{
    const dist = Vec2.Subtract(obj1.pos, obj2.pos).Magnitude();
    if (dist > obj1.hitBoxGeneric + obj2.hitBoxGeneric) return false;
    ...
    for (let i = 1; i < obj1.hitBox.length; i++)
    {
        ...
        for (let a = 1; a < obj2.hitBox.length; a++)
        {
            ...
            if (DoLinesCollide(line1, line2)) return true;
...
class PhysicsObject
{
    ...
    acceleration: Vec2;
    mass: number;
    hitBoxGeneric: number;
    hitBox: { x: number, y: number }[];
    world: ForgeLite | undefined;

    friction = 0.4;

    noGravity: boolean = false;
    noFriction: boolean = false;
    noCollide: boolean = false;

    #image: ImageData;

    constructor(pos: Vec2, rotation: Vec2, mass: number)
    {
        this.pos = pos;
    ...
    ApplyMotion(Direction: Vec2, Power: number, MassCalc = true)
    {
        if (MassCalc) this.acceleration.Add(Vec2.Multiply(Direction.Normalized(), Power / this.mass));
        else this.acceleration.Add(Vec2.Multiply(Direction.Normalized(), Power));
    };
};