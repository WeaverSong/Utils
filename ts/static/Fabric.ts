const Fabric = {
    Queue: <{[index: string]: {source: {}, func: Function}[]}>{},
    GlobalCatalogued: <any>[],
    fire: function (event: string, triggers = {})
    {
    ...
    subscribe: function (source: {}, event: string, func: Function)
    {
    ...
    unSubscribe: function (source: {}, event: string)
    {
        ...
        else
        {
            ...
            let item = ev.find(i => i.source === source);
            //If we failed to remove it, return false;
            if (item === undefined) return false;
            ev.splice(ev.indexOf(item), 1);
           return true;
        }
    },
    addEvent: function (name: string)
    {
        this.Queue[name] = [];
        return true;
    },
    ProxyRecursive: function (thing: {[index: string]: any}, path = "", override = function (item: {[index: string]: any}, key: string, thing: {[index: string]: any}) { return false })
    {
        let basePath = path;
    ...
    Proxy: function (name: string, object: {[index: string]: any}, path: string) {
